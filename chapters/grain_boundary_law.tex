...
There is however another way of solving the FE problem. In an implicit solver the equilibrium equation that tries to be solved aer
%
\begin{equation}
  \vec{K} \vec{u} - \vec{R} = 0.
\end{equation}
...
\caption{Comparison between the solution using either an explicit or implicit solver. This shows that for the region that the implicit solver converges the explicit and implicit solution agree well.}
\label{fig:impvsexp}
\end{figure}
